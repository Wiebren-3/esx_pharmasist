...
--function OpenCloakroomMenu()
	...
	local elements = {
		...
		{icon = "fas fa-shirt", title = TranslateCap('pharmasist_wear'), uniform = grade}
	}
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'pharmasist')

						TriggerServerEvent('esx_service:disableService', 'pharmasist')
						TriggerEvent('esx_pharmasistjob:updateBlip')
...
function OpenpharmasistActionsMenu()
	local elements = {
	...
	ESX.TriggerServerCallback('esx_pharmasistjob:getOtherPlayerData', function(data)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if data.current.value then
				TriggerServerEvent('esx_pharmasistjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('esx_pharmasistjob:getFineList', function(fines)
		local elements = {
			{unselectable = true, icon = "fas fa-scroll", title = TranslateCap('fine')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_pharmasist', TranslateCap('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle(elementF)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_pharmasistjob:getVehicleInfos', function(retrivedInfo)
				local elements = {
					{unselectable = true, icon = "fas fa-car", title = element.title},
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_pharmasistjob:getOtherPlayerData', function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			TriggerServerEvent('esx_pharmasistjob:message', GetPlayerServerId(player), TranslateCap('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_pharmasistjob:getArmoryWeapons', function(weapons)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local data = {current = element}
			ESX.TriggerServerCallback('esx_pharmasistjob:removeArmoryWeapon', function()
				ESX.CloseContext()
				OpenGetWeaponMenu()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_pharmasistjob:getStockItems', function(items)
		local elements = {
			{unselectable = true, icon = "fas fa-box", title = TranslateCap('pharmasist_stock')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_pharmasistjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_pharmasistjob:getPlayerInventory', function(inventory)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_pharmasistjob:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	if job.name == 'pharmasist' then
		Wait(1000)
		TriggerServerEvent('esx_pharmasistjob:forceBlip')
	end
end)
...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = TranslateCap('phone_pharmasist'),
		number     = 'pharmasist',
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'pharmasist' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_pharmasistjob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_pharmasistjob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_pharmasistjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'pharmasist' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_pharmasistjob:hasExitedEntityZone', function(entity)
...
-- Create blips
CreateThread(function()
	for k,v in pairs(Config.pharmasistStations) do
...
CreateThread(function()
	while true do
		local Sleep = 1500
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'pharmasist' then
			Sleep = 500
			local playerPed = PlayerPedId()
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_pharmasistjob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_pharmasistjob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
		...
		local Sleep = 1500
			...
			if closestDistance ~= -1 and closestDistance <= 3.0 then
				if LastEntity ~= closestEntity then
					TriggerEvent('esx_pharmasistjob:hasEnteredEntityZone', closestEntity)
					LastEntity = closestEntity
				end
			else
				if LastEntity then
					TriggerEvent('esx_pharmasistjob:hasExitedEntityZone', LastEntity)
					LastEntity = nil
				end
			end
		Wait(Sleep)
...
ESX.RegisterInput("pharmasist:interact", "(ESX pharmasistJob) Interact", "keyboard", "E", function()
	...
	elseif CurrentAction == 'menu_boss_actions' then
		ESX.CloseContext()
		TriggerEvent('esx_society:openBossMenu', 'pharmasist', function(data, menu)
			menu.close()
...
ESX.RegisterInput("pharmasist:quickactions", "(ESX pharmasistJob) Quick Actions", "keyboard", "F6", function()
	if not ESX.PlayerData.job or (ESX.PlayerData.job.name ~= 'pharmasist') or isDead then
		return
	end
...
RegisterNetEvent('esx_pharmasistjob:updateBlip')
AddEventHandler('esx_pharmasistjob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'pharmasist' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'pharmasist' then
...
AddEventHandler('esx:onPlayerSpawn', function(spawn)
	...
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_pharmasistjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_pharmasistjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'pharmasist')
